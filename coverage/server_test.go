	...
	cases := []testCaseErrors{
		{
			...
			Response:   ErrorBadRequest.Error(),
			StatusCode: 400,
		},
		{
			Url:        "https://127.0.0.1:8080/?limit=-5&offset=24&query=&order_field=Age&order_by=1",
			Response:   ErrorBadRequest.Error(),
			StatusCode: 400,
		},
		{
...
func TestSearchServerSpecificError(t *testing.T) {
	w := httptest.NewRecorder()
	SearchServer(w, nil)
	resp := w.Result()
	body, err := io.ReadAll(resp.Body)
